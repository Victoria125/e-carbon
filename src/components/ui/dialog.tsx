...
const DIALOG_OVERLAY_BASE_CLASSES = `
  fixed
  inset-0
  z-50
  bg-black/80
...
function DialogOverlay({ ref, className, ...props }: DialogOverlayProps) {
  return (
    <DialogPrimitive.Overlay
      ref={ref}
      className={cn(DIALOG_OVERLAY_BASE_CLASSES, className)}
      {...props} />
  )
}
DialogOverlay.displayName = DialogPrimitive.Overlay.displayName

const DIALOG_CONTENT_BASE_CLASSES = `
  fixed
  left-[50%]
  top-[50%]
  z-50
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ref={ref}
        className={cn(DIALOG_CONTENT_BASE_CLASSES, className)}
        {...props}>
        {children}
